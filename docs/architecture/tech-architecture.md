...
**职责**：用户交互界面，设备套件配套 App。双模式运行，共用安全体系。
...
### 6.1 双模式架构——独立模式 vs 联动模式

Feelings-App 不是必需的外设。它是可选的控制面板——用户在两种模式间自然切换，安全体系在两种模式下完全不变。

```
独立模式（没连手机）
    用户戴上设备。无需手机。设备完全自主。
    四诊合参直接采集身体数据。
    教练 AI 根据 PBM 状态决定给什么感受。
    "你不知道下一首是什么——但你知道它一定适合你。"
    和 MP3 模式完全同构。

联动模式（连接手机 App）
    打开 App → 三维图案展示当前等控器状态。
    用户点击图案 → 发起 session 请求。
    也可以自定义感受——选主旋律、点缀、强度。
    用户在教练 AI 的建议范围内有一部分控制权。
    和蓝牙模式完全同构。
```
...
```
独立模式
    用户控制权    零。戴上即开始。身体数据说了算。
    教练 AI      自主判断——根据四诊合参实时选择感受包和强度。
    安全体系      漏桶、阻尼、保底包全部在线——和联动模式完全相同。

联动模式
    用户控制权    在强度 ≤ cap 的范围内选择主旋律和点缀。
                不能自己造原子、不能改全局上限、不能关漏桶。
    教练 AI      建议感受方向和强度——用户必须确认。
                确认需要经过教练 AI 验证——
                不是"你想选什么就选什么"——
                是"你想选这个——你的 PBM 现在能不能接住"。
                AI 审核不过——信号不签发。强度自动降级。
    安全体系      与独立模式完全相同。漏桶照跑、不应期照样生效。
```

### 6.3 联动模式的启动流程

```
1. 打开App → 设备连接到手机 → 联动模式激活
2. 主面板显示三维图案——展示"你现在在哪"
   - 菱形呼吸节律、颜色、轮廓——实时 PBM 投影
3. 点击菱形中心 → 教练AI推送建议感受方向
   - "你现在可以试试平静。你的迷走神经昨晚恢复得很好。"
4. 用户可以选择接受建议——或自己调整主旋律+点缀
5. 确认 → 教练AI验证
   - 通过 → 信号签发 → 设备执行
   - 不通过 → 强度降级或替换替代感受包
6. 任何时候关闭App → 无缝切回独立模式。身体数据继续采集。安全体系不因模式切换而变。
```

### 6.4 三维状态图案

一个和骨传导耳机上的物理按钮同样简单的入口——当用户需要知道"我现在怎么样"时，打开App，看到的不是仪表盘，不是数据，是实时 PBM 投影的视觉锚点。点击即开始。不用看教程。不用选菜单。

所有复杂度——九 Pass 交织管线、漏桶、脱敏、阻尼、配置文件——藏在三维图案后面。用户只需要看到那一个菱形在呼吸。
...
### 八之附、内容与数据的两分法——感受包是内容，神经数据是数据

数据隐私架构的支柱是一个两分法：**神经数据是"数据"（个人的），感受包是"内容"（通用的）——数据绝对私有，内容可以分发。**

```
神经数据（数据）        个人的原始信号——绝对私有——
                        技术上不支持上传——永远在设备 + 家庭 NAS——
                        它是"一个人的记忆"

感受包（内容）          体验的配方——ESIR 帧参数模板——
                        registry 验证（质检）+ Feelings 打标签（分类）+
                        场景标注（一个人用或特定场景用）——
                        可上云分发——像应用商店、音乐库——
                        它是"大家共用的配方"

类比                    药方（感受包）：通用——分发——不同人效果不同
                        病历（神经数据）：私密——谁都不能看——
                        共享的是"怎么做"，永远不是"你做了什么"
```

**"每个人用效果也不一样，因为每个人的 PBM 不一样"——这正是内容与数据的分界线：** 感受包是通用配方，但配方在每个人身上的效果是个人化的——**内容可以公开，效果必须私有。** 所以：

```
上 Cloud    感受包（内容库）+ 感受包效果统计（脱敏的"有效/无效"分布）——
            Cloud = 内容库 + 统计 + 管理——公共知识
不上 Cloud  神经数据（个人原始信号）——设备 + 家庭 NAS——
            Cloud 碰不到"任何一个人的记忆"——
            它只持有"大家共用的配方"和"配方的群体效果"
...
**归属链条：感受包上 Cloud 的分发走 Feelings-Server（Registry 同步、感受包分发是 Server 职责）；神经数据在设备 Core 与家庭 NAS 的局域网闭环内。** 配方在公共空间流通，成品在私有空间存放——这套两分法，是数据主权架构在内容维度的完整实现。

---

## 九、当前阶段重点

> 2026-09-10 更新：当前阶段从"Layer 3 应用层"转为"**硬件层：监测与建基线**"。
> 依据：monitoring-first——调节哲学（过载者先降载、能量=恢复资源）需要个体基线做翻译层，
> 先让传感器数据流动、攒基线，再谈调节。见 `../../hardware/ROADMAP.md` 当前阶段。
> Layer 3（feelings-server/API）顺延为下一阶段。

```
现在（硬件层 · 监测与建基线）
  ✅ 执行端 ECP5 i5 全通（blinky + 串口回环 + flash 自启，2026-09-09）
  ✅ 采集端 iCE40 UART 回环
  ✅ 产品边界清晰（docs/product-boundary.md）
  ✅ GOVERNANCE 红线明确
  ⬜ 传感器 I2C 扫通（MAX30102/MPU6050，先 STM32 验证再搬 FPGA）
  ⬜ 持续采集 → 上传（开发期 PC / 产品期设备内）→ 建个体基线
  ⬜ 顶层设计对齐（本文档与 hardware/ROADMAP 的矛盾审查，进行中）
...
洗衣机几十种模式。绝大多数人一辈子只用混合洗。不是因为笨——是**那个模式已经覆盖了全部需求。**

Feelings 同理。Anim 九个 Pass。Core 五个模块。OS 六个守护进程。FPGA 物理约束。漏桶熔断。跨维度耦合。沙箱 Governance。SandboxResponse 标签。所有这些——用户永远不需要知道。

用户只需要知道一件事——"我轻松了。"

```
深度用户 → 挖得越深体验越丰富 → 每层都能自定义
普通用户 → 只需要按一个按钮 → 标准洗——够用
```

不是把复杂度降下来。是**把复杂度焊死在内部，外面只留一个钮。** 混合洗那个钮按下去——九个 Pass 自己跑、漏桶自己算、沙箱自己拦。没人需要知道洗衣机里面有 FPGA。
