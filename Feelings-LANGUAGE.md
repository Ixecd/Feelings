...
生命周期防悬垂         DefenceLevel D2+ 用户不能走标准路径
...
PSIR 的核心操作
    ...
    DefenceLevel 路径重路由
        defence: D1  →  所有非安全类型感受  →  交织期重路由至安全锚点
        （平静 / 归属 / 被理解，强度上限 15）
```
...
Pass 3: UserStateSafety（用户状态安全校验——依赖当前用户上下文）
     防御激活层级判定（DefenceLevel: D1/D2/D3——来自 Core PBM 生理信号）
     防御原子黑名单——不禁整个 feeling type，禁特定原子作为主旋律
         Shield 原子为主旋律 → 交织拒绝
         Shield 原子为点缀，配比尚在安全范围内 → 放行
         主旋律才触发生存级威胁，点缀只是自然出现的影子
         防御阶段不接受此感受类型 → 交织错误或静默重路由
     感受维度对当前用户 DefenceLevel 的安全兼容性检查
...
Preparing                     Pass 3 用户状态安全校验（DefenceLevel 判定、用户 cap）
                              加载 PBM，准备个性化适配
...
```
rule defence_protocol（防御激活协议——分级响应，信号驱动）
     defence 不是用户自述的叙事标签。来自 Core PBM 的生理信号——
     VSA 相变标记触发 D1/D2/D3 升级，非叙事驱动。

     D1 — 基础防御     禁主不禁点 (安全类外全禁)       强度上限不受影响
     D2 — 增强防御     禁主。点缀配比减半              强度 ≤ 50
     D3 — 极限防御     禁主。点缀全禁                  强度 ≤ 30

     Attack 协议:
      一次连续的 session 被判定为恶意违规（Governance G3触发）
      → DefenceLevel 从 None 升到 D1
      → 连续触发 → D1→D2→D3
      → D3 激活后——主动麻痹锚点——全禁 + Governance 接管

     DefenceLevel 升级由 Core PBM 状态机执行。
     Anim 只执行分级的安全约束——不判断、不诊断、不存档。
...
safety {
    ...
    // DefenceLevel D2 以上可访问，强度上限降至 20
    defence_level: v2,
    defence_intensity_cap: 20,
