...
rule trauma_protocol（创伤协议——分级分型）
    ...
    交叉判定矩阵
                   社交创伤    情绪创伤    躯体创伤
        v1          三重保守    三重保守    三重保守（均为安全三件套，不区分）
        v2          社交维度    负面点缀    本体感受强度限幅，可探索中性体感
                    强度 -10    配比 /2
        v3          社交维度    负面点缀    本体感受强度限幅，可谨慎扩展
                    强度 -10    配比 /2    物理参数 max × 0.5
...
Anim 不内嵌 if/for 流程控制——保持源码可全量静态审计。但通过声明式注解，创作者可以定义编译期展开的运行时安全行为。

```anim
// 注解在 feeling 或 shape 块级声明
@auto_reduce_on(heart_rate > 120)
    // 编译期展开 → ESIR 插桩：心率超 120 自动降强度

@auto_hold_on(respiration_irregular, tolerance(5s))
    // 呼吸不规则持续超过 5s → 暂停强度推进，保持当前参数

@auto_release_on(hrv_stabilized, tolerance(10s))
    // HRV 恢复稳定持续 10s → 解除自动减强度，慢速回升

@intensity_ceiling(50)
    // 此感受包的绝对强度上限（覆盖用户 cap，以较低者为准）

@recovery_required(between_sessions, 4h)
    // 此感受包体验结束后，4 小时内不能再次启动同类型 session

// 注解在编译期被展开为 ESIR 层的安全插桩代码。
// 不是运行时动态分支——是编译期代码生成。
// .anim 源码仍然可以被完整静态审计。
```
