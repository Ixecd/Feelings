...
```
feeling    感受原子——最基本不可再分的感受单元
            例：calm, joy, grief, fear, awe, belonging, love, loss

            feeling 本身不完整，需要上下文限定
            同样的 feeling 原子，不同参数 → 完全不同体验
```
...
Pass 2: SafetyCheck（安全编译期验证，不可跳过）
    强度 > cap → 编译错误
    点缀比例超出安全上限 → 编译错误
    创伤路径当前状态不接受此类感受 → 编译错误或静默重路由
...
Pass 4: Personalize（FSIR → PSIR）
    个人基线矩阵左乘
    强度 cap 二次验证（基线可能在编译过程中更新）
    创伤路径重路由生效
    闭环验证参数初始化

Pass 5: DeviceMap（PSIR → DSIR）
    设备分配矩阵
    设备缺失降级策略
    设备间时间同步偏移预计算
    音乐/声光协同信号生成

Pass 6: CodeGen（DSIR → ESIR）
    帧级参数生成
    闭环回读指针嵌入
    安全停止帧嵌入（每 N 帧插入安全校验点）
    紧急停止帧嵌入（硬件 kill 信号的软件镜像）
```

---

## 五、Anim 的安全模型——感受层面的借用检查
...
```
rule intensity_borrow
    "高强度区间的访问，必须先拥有对应低强度区间的已验证历史"
...
rule accent_ratio
    ...
    fear 作为点缀    上限 0.12   // 即使是点缀，恐惧也是恐惧
    grief 作为点缀   上限 0.20   // 悲伤点缀过重可能触发未处理创伤
    joy 作为点缀     上限 0.50   // 快乐的束缚比恐惧松散
    calm 作为点缀    无上限       // 平静是安全的基础色

    主旋律和点缀的配比 + 当前 cap + 用户 trauma 状态 → 编译期计算安全边界
    不同的主旋律，同样的点缀有不同的上限
    例：fear 点缀在「探索」中上限 0.12，在「韧性」中上限 0.25
```

### 5.3 形状安全约束

```
rule shape_intensity
    "形状类型与强度有对应安全要求"

    gradual_rise_fall    所有强度区间安全
    wave                 所有强度区间安全
    plateau              60 以下安全，以上需要渐进式引入
    double_peak          50 以下安全，以上需要评估
    delayed_burst        40 以下安全，以上需要知情
    aftershock           30 以下安全，以上需要创伤检查
    abrupt_stop          20 以下安全，以上需要明确知情同意
                         60 以上 → 编译期直接拒绝

    原因：abrupt_stop 的能量密度最高
    在没有任何准备的神经系统上突然断掉——是最容易造成冲击的形状
```
...
```
rule combo_limit
    "单个 session 中，不允许同时激活超过 2 个高危特征"

    高危特征包括
        intensity > 60
        shape 为 abrupt_stop / aftershock / delayed_burst
        mix 中含有 grief 或 fear 点缀且配比 > 0.10
        设备刚完成激活后首次 session

    任一 session 出现 3 个及以上高危特征 → 编译期拒绝
```

### 5.5 闭环安全兜底（运行期）

Anim 的编译期检查再严格，也无法预测实时生理响应。以下情况发生时，**运行时可触发安全插桩**：

```
触发条件                    响应
─────────────────────      ─────────────────────
心率超出预期区间 30%+      下一帧自动降强度 20%，持续 10 帧
皮电骤然峰值（2s 内 50%+） 暂停当前 shape，插入平静基准帧
呼吸频率 < 正常 60%        暂停 session，切换至冷静模式
三种信号同时异常            保底包激活，session 终止
```

**编译期检查是墙，运行期插桩是气囊。两层都要，一层都不能缺。**
...
```
新用户，0 session → PBM 为人群均值 + 最大保守偏移

    人群均值 PBM₀    基于同类设备、同年龄段的统计数据
    最大保守系数    所有偏移方向都偏向更保守（刺激更弱、强度更低）

    例：FSIR 要求 0.5mA
        此用户 PBM：人群均值系数 1.0
        但冷启动附加系数 0.6
        → 实际 0.3mA，然后观察 5 次 session 的反应
        → 逐渐放开系数，直到第 50 次 session 收敛到个人真实偏移
```
