...
Anim 的编译器强制保证——在这些规则被违反时，代码不通过。不是运行时报警，是压根不生成信号。
...
> **强度刻度的设计决策**：当前 `.anim` 源码中强度区间使用 0-100 整数。这不是上限——底层 `Intensity.min/max` 是 `u32`——可以写 0-1000、0-10000——语法解析不拦。选 100 作为默认刻度有三个原因：第一，整数在 FPGA 寄存器里比较只需要一拍——浮点需要 DSP Block 或多个周期——1ms 帧确定性优先。第二，创作者的大脑负担——"37" 比 "37.5" 轻——少一个字符——合起感受更快。第三，100 档在几十万 session 数据积累之前完全够用。未来如果需要更高精度——Pattern Registry 里的原子声明"我推荐精度 0-1000"——设备固件归一化——不需要改语法、编译器或 IR。刻度变了——比例没变。
...
Pass 2: StaticSafety（静态安全规则校验——不可跳过，不可降级）
    强度 > cap → 编译错误（此处 cap 为感受包的绝对上限声明）
...
沙箱做的事
    ...
    限制
        强度硬上限 ≤ 30（编译器 Pass 1 拒绝更高强度的沙箱原子）
    ...
    签名
        沙箱原子编译产物带有 unverified_sandbox 签名位
...
animi 交织器最特殊的地方在于：**同一份 .anim 源码，在不同用户身上，编译结果完全不同。**
...
animi 交织器  编译的是 10⁹ bit/s 域的信号参数
