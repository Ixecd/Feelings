...
现状（**10 列**——已并入 accel + 皮温）：

```
t, idx, red, beat, ibi_samples, ibi_ms, ax, ay, az, temp_c
```
...
| `temp_c` | ✅ 已落 CSV | TMP117 皮温（°C，i16×0.0078125，≈1Hz，0.0078°C 分辨率） |
...
| TMP117（皮温） | ✅ 已接入：流固件发 `FE E4`(温度)+`FE E5`(器件ID 期望 0x0117)；PC 出 `temp_c`（≈1Hz） |
| 硬件时基 / 长时环形存储 | ⬜ 路线图后续（见 ROADMAP） |

---

## 七、多模态测量集（从"HRV 基线"升级为"多模态基线"）

tVNS 打的不是一个"心率开关"——迷走耳支 → 孤束核 → 蓝斑/中缝核/下丘脑，牵动整个自主网络。
输出是一大串，**HRV 只是"心率侧"的一个便宜代理**。所以测量要从单模态升级：

| 通道 | 信号 | 测什么 | 主观可感？ | 状态 |
|---|---|---|---|---|
| 心脏 | HR / HRV | 迷走→心脏 | 心跳、心慌 | ✅ 现在 |
| 皮肤电 | EDA / GSR | 交感汗腺（情绪/唤醒的经典指标） | 手心出汗、紧张发麻 | ⬜ 加（几块钱电极） |
| 皮温/血流 | 皮温 | 血管运动 | 冷/暖、手脚凉 | ✅ TMP117（已接入，FE E4） |
| 呼吸 | 速率/变异 | 呼吸性窦性心律不齐 | 能感到呼吸 | 🟡 可从 PPG/accel 推 |
| 瞳孔 | 瞳孔直径 | 自主神经 | 难自控 | ⬜ 以后 |
| 皮层 | EEG（α/θ） | 中枢效应 | 难自感 | ⬜ 以后 |
| 免疫 | 炎症因子 | 慢性 | 无 | ⬜ 长期 |
| **主观** | 平静/情绪 | **"效果"本身** | ★ 就是主观 | ✅ 盲后自评 |

**两条纪律：**

1. **主观是多模态的一部分，不是替代。** 很多自主信号**本来就主观可感**（出汗、冷热、心跳、呼吸），所以自评是合法通道——但**必须盲**（知道在打 → 期待污染）。
2. **"找到你的响应通道"**——tVNS 急性响应个体差异大：有人 HRV 变、有人不变但 EDA 或主观有反应。**只盯 HRV 会漏人**；多模态就是为了找到"你走哪条"。

```
MVP 优先级:  HR/HRV + accel + 皮温（有） → EDA → 主观(盲) → 呼吸(软推) → EEG(以后)
```

---

*基线不是"多采几次 HRV"就行——是"多模态地采、在真实情境里采、并且记下来"。这份协议管的就是这件事。*
