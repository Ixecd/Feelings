...
**坑五：波特率对不上 → 计数器溢出。** `reg [9:0] baud_cnt` 只有10位宽——最大值1023。9600波特时DIV=625（未溢出），300波特时DIV=20000（溢出——计数器永远到不了——baud_clk从未翻转——TX从未发送——RXD LED全程灭）。改成 `reg [14:0] baud_cnt`（15位宽）后——300波特立即稳定回显。9600波特用同样的15位宽计数器能够慢打回显、快打偶有乱码——已在功能基准层面验证通过。

**最终状态: ✅ UART回环功能基准通过。** 300波特稳定无误。9600波特可用——高速模式下RX采样需要进一步优化——但第一帧echo已成功送达。

---

## 2026-07-20 后续优化——内部振荡器+单时钟域+原始周期计数

**核心发现：iCE40UP5K内部HFOSC实际运行在48MHz（不是24MHz），CLKHF_DIV="0b00" 不分频。**

**坑六：外部12MHz时钟不可靠。** 之前12MHz的外部STM32 MCO时钟在反复烧录后不稳定，LED闪烁器可以跑但UART始终不通。切到内部SB_HFOSC后FPGA运行完全正常。

**坑七：48MHz还是24MHz。** 最初误判HFOSC为24MHz（通过间接波特率反推）。实际是48MHz。用LED心跳闪烁（cnt[25]）可肉眼确认频率——约0.72Hz心跳=48MHz。

**坑八：多always块握手。** TX和RX分属不同always块时，单周期`done`脉冲可能被TX漏掉。最终把所有逻辑合并到单个always块——RX收完直切TX——零握手——可靠。

**最终方案：**
- 时钟：SB_HFOSC 48MHz（CLKHF_DIV="0b00"）
- 波特率：9600，48M/9600=5000周期/bit
- 架构：单always块——状态机：0空闲→1半起始位→2数据位→3-12发送位→0空闲
- 引脚：RX=FPGA ball 2（对应PMOD P1_11），TX=FPGA ball 6（对应PMOD P1_2），LED=39
- 使用率：128/5280 LC（2%）
- 已知限制：快速连续打字时单缓冲偶尔丢帧——因为TX传输期间不监听RX
